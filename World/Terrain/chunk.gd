...
signal collision_created

#Terrain size
@export_range(20,1000, 1)var Terrain_Size := 200
#LOD scaling
@export_range(1, 100, 1) var resolution := 50
@export var Terrain_Max_Height = 25
#set the minimum to maximum lods 
#to change the terrain resolution
@export var chunk_lods : Array[int] = [2,4,8,16,25,50]
@export var LOD_distances : Array[int] = [3594,2995,2396,1797,1198,601]
...
func generate_terrain(noise:FastNoiseLite,coords:Vector2,size:float,initailly_visible:bool):
	...
	var water = preload("res://World/Terrain/water.tscn").instantiate()
	add_child(water)
	water.global_position.y = -1
...
func _on_collision_created():
	for x in get_children(true):
		if x.name.substr(0,5) == "Chunk":
			x.set_collision_layer_value(2, true)
			x.set_collision_mask_value(2, true)
